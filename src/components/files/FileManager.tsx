...
import {
  ...
  FileJson, FileSpreadsheet, Presentation, Eye, Trash2, Info,
  Grid, List, Search, ChevronRight, Plus, X, HardDrive, ArrowLeft, Edit2, FilePlus
} from "lucide-react";
    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          <HardDrive className="h-5 w-5 shrink-0 text-slate-400" aria-hidden="true" />
        ...
        <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:justify-end">
          <div className="relative min-w-0 flex-1 sm:flex-none">
            ...
            <Input
              ...
              className="h-8 w-full border-white/10 bg-white/5 pl-8 text-xs sm:h-7 sm:w-40"
            />
          </div>
          <div className="flex items-center rounded-md border border-white/10 bg-white/[0.03] p-0.5">
            <button type="button" onClick={() => setViewMode("list")} aria-pressed={viewMode === "list"} className={cn("flex h-7 w-7 items-center justify-center rounded text-xs sm:h-6 sm:w-6", viewMode === "list" ? "bg-white/10 text-white" : "text-slate-500")} aria-label="List view"><List className="h-3.5 w-3.5" /></button>
            <button type="button" onClick={() => setViewMode("grid")} aria-pressed={viewMode === "grid"} className={cn("flex h-7 w-7 items-center justify-center rounded text-xs sm:h-6 sm:w-6", viewMode === "grid" ? "bg-white/10 text-white" : "text-slate-500")} aria-label="Grid view"><Grid className="h-3.5 w-3.5" /></button>
          </div>

          <Button size="sm" onClick={() => setShowCreateDoc(true)} className="h-8 flex-1 gap-1 bg-cyan-500 text-xs font-semibold text-black hover:bg-cyan-400 sm:h-7 sm:flex-none">
            <FilePlus className="h-3.5 w-3.5" aria-hidden="true" />
            <span className="sm:inline">Create</span>
            <span className="hidden sm:inline"> Document</span>
          </Button>

          <Button size="sm" onClick={() => setShowUpload(!showUpload)} className="h-8 flex-1 gap-1 bg-cyan-500 text-xs font-semibold text-black hover:bg-cyan-400 sm:h-7 sm:flex-none">
            {showUpload ? <X className="h-3.5 w-3.5" aria-hidden="true" /> : <Plus className="h-3.5 w-3.5" aria-hidden="true" />}
      ...
      {/* Content */}
      {files === undefined ? (
        <div className="space-y-2">{[1, 2, 3, 4, 5].map((i) => <Skeleton key={i} className="h-12 rounded-lg bg-white/5" />)}</div>
      ) : files.length === 0 ? (
        <EmptyState
              ...
              <div key={file._id} className="group flex items-center gap-3 rounded-lg border border-white/[0.06] bg-[#0c1222]/60 px-3 py-2 transition-colors hover:border-white/[0.1] hover:bg-[#0c1222]">
                ...
                <div className="flex shrink-0 items-center gap-0.5 opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100">
                  {isPreviewable(file.type, file.name) && (
                    <button type="button" onClick={() => setPreviewFile(file)} className="flex h-7 w-7 items-center justify-center rounded text-slate-400 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/40" aria-label={`Preview ${file.name}`}><Eye className="h-3.5 w-3.5" /></button>
                  )}
                  <button type="button" onClick={() => { setRenameFileItem(file); setNewFileName(file.name); }} className="flex h-7 w-7 items-center justify-center rounded text-slate-400 hover:bg-cyan-500/10 hover:text-cyan-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/40" aria-label={`Rename ${file.name}`}><Edit2 className="h-3.5 w-3.5" /></button>
                  <button type="button" onClick={() => setSelectedFile(file)} className="flex h-7 w-7 items-center justify-center rounded text-slate-400 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/40" aria-label={`Details for ${file.name}`}><Info className="h-3.5 w-3.5" /></button>
                  <button type="button" onClick={() => setDeleteConfirm(file)} className="flex h-7 w-7 items-center justify-center rounded text-slate-400 hover:bg-red-500/10 hover:text-red-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/40" aria-label={`Delete ${file.name}`}><Trash2 className="h-3.5 w-3.5" /></button>
                </div>
              </div>
            );
          })}
          {search && fileItems.length === 0 && (
            <p className="py-8 text-center text-xs text-slate-500">No files match your search.</p>
          )}
        </div>
      ) : (
        /* Grid view */
