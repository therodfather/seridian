# Dashboard pages

Each page has a short doc: what it does, which files to edit, which flow it uses.

## Hubs (in sidebar)

| Page | Doc |
|------|-----|
| Home / Overview | [overview.md](./overview.md) |
| Business hub | [business.md](./business.md) |
| Knowledge hub | [knowledge.md](./knowledge.md) |
| Work (Issues) | [issues.md](./issues.md) |
| Voice / IVR | [ivr.md](./ivr.md) |
| Settings | [settings.md](./settings.md) |

## Nested under Business (not in sidebar)

| Page | Doc |
|------|-----|
| Clients | [clients.md](./clients.md) |
| Proposals | [proposals.md](./proposals.md) |
| Sales | [sales.md](./sales.md) |
| Contracts | [contracts.md](./contracts.md) |
| Bookings | [bookings.md](./bookings.md) |
| Health Check | [health-check.md](./health-check.md) |

## Nested under Knowledge (not in sidebar)

| Page | Doc |
|------|-----|
| Wiki | [wiki.md](./wiki.md) |
| Second Brain | [brain.md](./brain.md) |
| Files | [files.md](./files.md) |
| Templates | [templates.md](./templates.md) |
| Chat | [chat.md](./chat.md) |
| LLM Arena | [arena.md](./arena.md) |

## Other

| Page | Doc |
|------|-----|
| Sync (redirect) | [sync.md](./sync.md) |

Back → [Dashboard index](../index.md)
